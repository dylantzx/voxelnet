...
class MiddleAndRPN:
    def __init__(self, input, alpha=1.5, beta=1, sigma=3, training=True, name=''):
        ...
        with tf.compat.v1.variable_scope('MiddleAndRPN_' + name):
            ...
            self.cls_loss = tf.compat.v1.reduce_sum( alpha * self.cls_pos_loss + beta * self.cls_neg_loss )
            self.cls_pos_loss_rec = tf.compat.v1.reduce_sum( self.cls_pos_loss )
            self.cls_neg_loss_rec = tf.compat.v1.reduce_sum( self.cls_neg_loss )
...
def smooth_l1(deltas, targets, sigma=3.0):
    ...
    smooth_l1_signs = tf.compat.v1.cast(tf.compat.v1.less(tf.compat.v1.abs(diffs), 1.0 / sigma2), tf.float32)
